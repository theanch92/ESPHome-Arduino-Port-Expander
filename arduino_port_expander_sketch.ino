...
void setup() {
  ...
  emon1.voltage(0, 234.59, 0.9);
  emon1.current(1, 77.1);
  emon2.voltage(0, 234.49, 0.9);
  emon2.current(2, 77.1);
  emon3.voltage(0, 258.69, 0.9);
  emon3.current(3, 64.8);
}
...
#define DIGITAL_READ(b, pin, mask) \
  if (digitalRead(pin)) buffer[b] |= mask;
...
void handleTextCommand(uint8_t id) {
  switch (id) {
    case 1:  // getTemp
    {
      String h = String(realPower1, 2);
      h.getBytes((unsigned char *)(&buffer), 9);
    } break;
    case 2:  // getHumidity
    {
      String hu = String(realPower2, 2);
      hu.getBytes((unsigned char *)(&buffer), 9);
    } break;
    case 3:  // getHeatIndex
    {
      String hi = String(realPower3, 2);
...
void onReceive(int numBytes) {
  ...
  if (cmd >= CMD_READ_TEXT &&
      cmd <= (CMD_READ_TEXT + 32))  // 32 is arbitrary limit
  {
